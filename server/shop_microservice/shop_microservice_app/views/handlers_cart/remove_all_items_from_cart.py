...
def remove_all_items_from_cart(request, product_id):
    ...
    try:
        ...
        cart_item.delete()
        return Response({"message": "Товары успешно удален из корзины"}, status=status.HTTP_204_NO_CONTENT)
    except CartItem.DoesNotExist:
        return Response({"error": "Товары не найден в корзине"}, status=status.HTTP_404_NOT_FOUND)
